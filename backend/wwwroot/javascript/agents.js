let homeBtn, loginBtn, logoutBtn;
...
function updateHeaderAuthUi(user) {
  ...
  homeBtn?.classList.toggle("hidden", isLoggedIn);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderPagination() {
    pagination
      .querySelectorAll(
        ".page-btn:not([data-page='prev']):not([data-page='next'])",
      )
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pagination.insertBefore(
        btn,
        pagination.querySelector("[data-page='next']"),
      );
    }
  }
  ...
  sendBtns.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (!photo || !name || !role || !agency || contactPs.length < 3) return;
      ...
      modalSocial.textContent =
        "Instagram: @" + name.textContent.split(" ")[0].toLowerCase();
  ...
  logoutBtn?.addEventListener("click", async () => {
    try {
      ...
      updateHeaderAuthUi({ isLoggedIn: false });
